...
| macOS | [![Download for macOS](https://img.shields.io/badge/Download-macOS-blue?logo=Apple&logoColor=white)](https://github.com/orffen/augustus-updater/releases/latest/download/augustus-updater-mac.dmg) |
...
On macOS, it will store all of its files (including the `Augustus Unstable.app`) in `~/Library/Application Support/Augustus Updater`. Note that you may need to modify its permissions to allow it to execute. You can do this by running:
```bash
